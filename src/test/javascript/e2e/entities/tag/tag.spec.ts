...
describe('Tag e2e test', () => {
  ...
  it('should load create Tag page', async () => {
    ...
    expect(await tagUpdatePage.getPageTitle().getAttribute('id')).to.match(/platformApp.tag.home.createOrEditLabel/);
  });

  it('should create and save Tags', async () => {
    const nbButtonsBeforeCreate = await tagComponentsPage.countDeleteButtons();

    await tagUpdatePage.setTagIdInput('tagId');
    expect(await tagUpdatePage.getTagIdInput()).to.match(/tagId/);
    await tagUpdatePage.setRemainingBatteryInput('5');
    expect(await tagUpdatePage.getRemainingBatteryInput()).to.eq('5');
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/platformApp.tag.delete.question/);
